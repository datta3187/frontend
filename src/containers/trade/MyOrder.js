...
import {fetchOpenOrders} from "../../redux/actions/trade";
import {connect} from "react-redux";
import { ORDER_WAIT, ORDER_LIMIT } from '../../redux/constants/trade';
...
class connectedMyOrder extends Component{
    ...
    componentWillMount() {
        this.props.fetchOpenOrders({ market: this.props.market, state: ORDER_WAIT, limit: ORDER_LIMIT })
    }

    componentWillReceiveProps(nextProps) {
        this.setState({orders: nextProps.OpenOrders});
    }
...
function mapDispatchToProps(dispatch){
    return {
        fetchOpenOrders: (payload) => dispatch(fetchOpenOrders(payload))
    }
}

const MyOrder= connect(mapStateToProps, mapDispatchToProps)(connectedMyOrder);
